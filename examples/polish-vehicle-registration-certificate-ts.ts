#!/usr/bin/env ts-node

import nrv2eDecompress from '../lib/nrv2e-decompress'

// Scanned from https://www.pwpw.pl/binaries/content/gallery/pwpwcms_pl/Dla_mediow/zdjecia_do_artykulow/2016/03/dsc_1548.jpg
const b64Input: string = ''
const binInput: Buffer = Buffer.from(b64Input, 'base64')

// 4 first bytes are output length
const outputLength: number = binInput.readUInt32LE(0)
const utf16Output: Buffer = Buffer.alloc(outputLength)
const compressedInput: Buffer = binInput.slice(4)

nrv2eDecompress(compressedInput, utf16Output)

const textOutput: string = utf16Output.toString('utf16le')

console.info(textOutput)
